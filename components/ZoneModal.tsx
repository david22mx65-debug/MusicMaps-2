...
const ZoneModal: React.FC<ZoneModalProps> = ({ location, onSave, onCancel, language, uiTheme, uiStyle }) => {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-2 sm:p-4">
      <div className={`${uiStyle === 'pixel' ? (uiTheme === 'light' ? 'bg-[#fdf8f5]' : 'bg-[#1a1c1e]') : (uiTheme === 'light' ? 'bg-[#f5f5f5]' : 'bg-[#121212]')} border ${uiTheme === 'light' ? 'border-black/5' : 'border-white/5'} w-full sm:max-w-md rounded-t-[32px] sm:rounded-[32px] p-6 sm:p-8 shadow-[0_20px_60px_rgba(0,0,0,0.8)] flex flex-col gap-4 sm:gap-6 animate-in slide-in-from-bottom-20 duration-500 max-h-[95vh] sm:max-h-[90vh] overflow-y-auto custom-scrollbar`}>
        
        <div className="flex justify-between items-center">
          <h2 className={`text-xl sm:text-2xl font-black ${uiTheme === 'light' ? 'text-black' : 'text-white'} tracking-tighter`}>
            {t.title}
          </h2>
          <button onClick={onCancel} className={`p-2 ${uiStyle === 'pixel' ? 'bg-primary/20 text-primary' : (uiTheme === 'light' ? 'hover:bg-black/5 text-black/40' : 'hover:bg-[#282828] text-[#B3B3B3]')} rounded-full transition-colors`}>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <input 
                ...
                className={`flex-1 ${uiStyle === 'pixel' ? (uiTheme === 'light' ? 'bg-primary/10 text-black' : 'bg-primary/5 text-white') : (uiTheme === 'light' ? 'bg-black/5 text-black placeholder:text-black/20' : 'bg-[#282828] text-white placeholder:text-[#535353]')} border-none rounded-2xl p-3 sm:p-4 text-sm sm:text-base focus:ring-2 focus:ring-primary outline-none font-bold`}
              />
              <button 
                onClick={handleGeminiSuggestion}
                disabled={isGenerating}
                className={`${uiStyle === 'pixel' ? 'bg-primary/20 hover:bg-primary/30' : (uiTheme === 'light' ? 'bg-black/5 hover:bg-black/10' : 'bg-[#282828] hover:bg-[#3e3e3e]')} p-3 sm:p-4 rounded-2xl text-primary disabled:opacity-50 transition-all border ${uiTheme === 'light' ? 'border-black/5' : 'border-white/5'}`}
                title={t.suggestTip}
              >
          ...
          <div className={`${uiStyle === 'pixel' ? (uiTheme === 'light' ? 'bg-primary/10' : 'bg-primary/5') : (uiTheme === 'light' ? 'bg-black/5' : 'bg-[#181818]')} p-4 sm:p-6 rounded-3xl border ${uiTheme === 'light' ? 'border-black/5' : 'border-white/5'} space-y-3 sm:space-y-4`}>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
                <div className="relative group">
                    ...
                    />
                    <div className={`h-full flex flex-col items-center gap-2 p-3 sm:p-4 ${uiStyle === 'pixel' ? 'bg-primary/10' : (uiTheme === 'light' ? 'bg-black/5' : 'bg-[#282828]')} rounded-2xl border border-transparent group-hover:border-primary transition-all text-center justify-center min-h-[80px] sm:min-h-[100px]`}>
                ...
                <div className="relative group">
                    <input 
                      ...
                      className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                    />
                    <div className={`h-full flex flex-col items-center gap-2 p-3 sm:p-4 ${uiStyle === 'pixel' ? 'bg-primary/10' : (uiTheme === 'light' ? 'bg-black/5' : 'bg-[#282828]')} rounded-2xl border border-transparent group-hover:border-primary transition-all text-center justify-center relative overflow-hidden min-h-[80px] sm:min-h-[100px]`}>
                      {coverImage ? (
